...
var SRC string = "mmc/src/go/xmlappread.go"
...
var reQ = regexp.MustCompile("Successfully sent a JMS message. Type: ObjectMessage Destination: (TFMSJMS/[A-Z|a-z|0-9|_]*) (Properties:.*)")
...
func process(FILE string) {
	...
	first_row := ""
	if *mfiles != "no" {
		m := strings.Split(FILE, "/")
		if len(m) > 0 {
			k := strings.Split(m[len(m)-1], ".")
			first_row = fmt.Sprintf("%v.%v,", k[0], k[1])
		}
	}
	...
	for {
		...
		case xml.StartElement:
			...
			if inElement == "event" {
				...
				// Consistent number of cols
				tmp := strings.Split(Con(p.Data), ",")
				for ii := 4; ii > len(tmp); ii-- {
					p.Data = p.Data + ","
				}
				// Put an if statement here
				if reQ.MatchString(ConR(p.MS)) {
					if 1 == 2 {
				  fmt.Printf("%s%v,%v,%v,\"%v\"\n",
					first_row, TimeParse(p.Timestamp), p.Epoch, Con(p.Data), ConR(p.MS))
                                        }

					fmt.Printf("%v,%v\n",p.Epoch,reQ.FindStringSubmatch(ConR(p.MS))[1])
				}
