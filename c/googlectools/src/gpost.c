...
int mygetopt(int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "he:p:vs:f:i:a:",
		...
		case 'h':
			flags |= 0x1;
			printf("gpost -s <site> -a <page>\n"
			       "Example:  gpost -s www.cwxstat.org\n");
			exit(0);
			break;
	...
	if (~flags & 0x90) {
		fprintf(stderr,
			...
			"Usage:\n\tgpost  -s <site> -a <page>\n"
			"Example:\n\tgcookie -i .gauthrc -s dev.mchirico.org -a /cron/fireAction\n");
		exit(2);
	}
