...
static ssize_t socket_write(int sockfd, char *_sendline)
{
	ssize_t n = 0;
...
char *mygetopt(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'h':
			flags |= 0x1;
			printf
			    ("glogin -e <email> -p <password> -s <site>\n"
			     "Example:  ./glogin -e billybob -p Pa33word -s www.cwxstat.org\n");
			exit(0);
			break;
