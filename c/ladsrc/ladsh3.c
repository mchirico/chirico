/* ladsh3.c - Third shell version, adding globbing and directory handling */
    ...
    while (*src && !done) {
        ...
        case '<':
            ...
            if (buf != prog->argv[argc]) {
                /* the stuff before this character may be the 
		   file number being redirected */
...
void checkJobs(struct jobSet * jobList) {
    ...
    while ((childpid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if (!job->runningProgs) {
            ...
            printf(JOB_STATUS_FORMAT, job->jobId, msg, job->text);
