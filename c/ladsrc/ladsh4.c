/* ladsh4.c - The complete version of a basic system shell */
...
struct childProgram {					    
    ...
    struct redirectionSpecifier * redirections;  /* I/O redirs */
    glob_t globResult;      /* result of parameter globbing */
    int freeGlob;           /* should we free globResult? */
    int isStopped;          /* is the program currently running? */
};							    

struct job {						    
    int jobId;              /* job number */
    int numProgs;           /* number of programs in job */
    ...
    int stoppedProgs;       /* num of programs alive, but stopped */
};							    
...
void globLastArgument(struct childProgram * prog, int * argcPtr,
    ...
    if (argc > 1) {        /* cmd->globResult already initialized */
        flags = GLOB_APPEND;
    ...
    while (*src && !done) {
        ...
        case '<':
            ...
            if (buf != prog->argv[argc]) {
                /* the stuff before this character may be 
		   the file number being redirected */
...
int setupRedirections(struct childProgram * prog) {
    ...
    for (i = 0; i < prog->numRedirections; i++, redir++) {
        ...
        if (openfd < 0) {
            /* this could get lost if stderr has been redirected, 
	       but bash and ash both lose it as well (though zsh 
	       doesn't!) */
...
int runCommand(struct job newJob, struct jobSet * jobList, 
    ...
    char * statusString;
    int jobNum;
    int controlfds[2];		/* a pipe to make the child pause */

    /* handle built-ins here -- we don't fork() so we 
       can't background these very easily */
    if (!strcmp(newJob.progs[0].argv[0], "exit")) {
        ...
        if (!newJob.progs[0].argv[1] || newJob.progs[0].argv[2]) {
            fprintf(stderr, 
		    "%s: exactly one argument is expected\n",
	...
	pipe(controlfds);					
	    ...
	    close(controlfds[1]);
	    /* this read will return 0 when the write side closes */
	...
	/* close the control pipe so the child can continue */
	close(controlfds[0]);
	close(controlfds[1]);
