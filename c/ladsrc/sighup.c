/* sighup.c - Demonstration of \codefn{SIGHUP} forcing log file resets */
...
volatile int reopenLog = 0;     /* volatile as it is modified 
				   by a signal handler */
...
int main() {
    ...
    /* Set up a signal handler for SIGHUP. Use memset() to 
       initialize the struct sigaction to be sure we clear all 
       of it. */
    ...
    while (!done) {
        ...
        if (rc) {
            if (reopenLog) {
                logstring(logfd, 
		    "* reopening log files at SIGHUP's request\n");
                reopenLog = 0;
            } else {
                logstring(logfd, 
		    "* sleep interrupted by unknown signal "
                          "-- dying\n");
                done = 1;
            }
        } else {
